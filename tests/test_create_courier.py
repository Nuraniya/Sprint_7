...
class TestCreateCourier:
    ...
    def test_cannot_create_duplicate_courier(self, cleanup_courier):
        ...
        with allure.step("Авторизоваться под первым курьером"):
            login_payload = {"login": login, "password": password}
            login_response = requests.post(URLs.COURIER_LOGIN, json=login_payload)
            assert login_response.status_code == 200
    ...
    @allure.title("Тест: создание курьера без обязательного поля возвращает ошибку")
    ...
    def test_create_courier_with_existing_login_returns_error(self, cleanup_courier):
        login = generate_random_string(10)
        password = generate_random_string(10)
        first_name = generate_random_string(10)
